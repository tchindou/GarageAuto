...
$data = \App\Models\Garage::find($index);
$eventId = $index;

$border = '';

if ($data->boost >= 7) {
    $border = 'outline outline-1 outline-offset-2 outline-yellow-500';
    }
@endphp

<div class='desc image-dialog-{{ $index }} relative flex h-64 m-auto mb-8 cursor-pointer flex-col overflow-hidden  rounded-xl bg-gray-500 bg-clip-border text-gray-50 transition ease-in-out delay-150 hover:translate-y-1 hover:scale-100 duration-300 {{ $border }}'
    data-dialog-target="image-dialog-{{ $index }}">

    <img alt="{{ $data->name }}" class="h-full w-full object-fill object-center" src="{{ $data->image }}" />

    <div class="flex flex-col justify-between bg-black bg-opacity-50 absolute inset-0 h-full w-full px-1 py-1">

        <div class="relative flex flex-row justify-between pr-1 w-full">

            <div class="flex gap-1 items-center">
                <span class="relative flex h-0.5 w-0.5">
                    <span
                        class="animate-ping absolute inline-flex h-full w-full rounded-full bg-green-400 opacity-75"></span>
                    <span class="relative inline-flex rounded-full h-0.5 w-0.5 bg-green-500"></span>
                </span>
                <span class="text-sm text-green-500">A venir</span>

            </div>
            <span class="shrink"></span>
            <div
                class="flex flex-row justify-between bg-gray-600 opacity-80 h-fit w-fit p-0.5 items-center gap-1 rounded-md">
                <span class="text-sm text-white">1.1k</span>
        ...
        <div class="relative block w-full">
            ...
            <span class="line-clamp-1 md:text-xl sm:text-lg text-base text-white mb-3">{{ $data->name }}</span>

            <div class="desc-item group-hover:flex hidden mb-2 ml-1 gap-1">

                <div
                    class=" overflow-hidden min-w-fit max-w-[16.5%] whitespace-nowrap overflow-ellipsis outline outline-offset-2 outline-1 outline-blue-500 flex flex-row justify-between bg-gray-800 opacity-60 h-fit w-fit mr-2 p-0.5 items-center gap-1 rounded-md">
                    ...
                    <span class="text-xs text-white">{{ $data->addresse }}</span>
                </div>

                <div
                    class="outline outline-offset-2 outline-1 outline-blue-500 flex flex-row justify-between bg-gray-800 opacity-60 h-fit w-fit mr-2 p-0.5 items-center gap-1 rounded-md">
                    ...
                    <span class="text-xs text-white">{{ $data->dateEvenement }}</span>
                </div>
                ...
                <div
                    ...
                    <span class="text-xs text-white">150</span>
                </div>
...
<div data-dialog-backdrop="image-dialog-{{ $index }}" data-dialog-backdrop-close="true"
    ...
    <div class="z-[100] relative m-4 min-w-[90%] sm:min-w-[90%] sm:max-w-[90%] md:min-w-[80%] md:max-w-[80%] lg:min-w-[70%] lg:max-w-[70%] max-w-[90%]  rounded-lg bg-white font-sans text-base font-light leading-relaxed antialiased shadow-2xl"
        role="dialog" data-dialog="image-dialog-{{ $index }}">
        ...
        <div class="flex flex-col items-start gap-1 p-4">
            ...
            <div class="flex flex-row w-full justify-end gap-3">
                ...
                <button id="shareBtn" data-share-target="share-{{ $index }}"
            ...
            <div class=" mb-5">
                ...
                <div class=" pl-3">
                    <p class="md:text-base line-clamp-3">{{ $data->description }}</p>
                </div>
            </div>
            ...
            <div class="gap-3 mb-5">
                ...
                <span class="text-base md:text-lg text-underline"><a href="#">{{ $data->addresse }}</a></span>
            </div>
